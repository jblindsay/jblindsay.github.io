...
/**
 * This tool creates a new raster in which each grid cell is equal to the integer division of the corresponding grid cells in two input rasters or one input raster and a constant value.
 *
 * @author Dr. John Lindsay email: jlindsay@uoguelph.ca
 */
public class IntDiv implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Performs a integer division operation on two rasters or a raster and a constant value.";
    }
     ...
     * Whitebox user interface.
     *
     * @param progress int containing the progress value (between 0 and 100).
     */
    ...
    public void run() {
        ...
        try {
            ...
            if (image1Bool && image2Bool) {
                ...
                for (row = 0; row < rows; row++) {
                    ...
                    for (col = 0; col < cols; col++) {
                        ...
                        if ((z1 != noData1) && (z2 != noData2)) {
                            if (z2 != 0) {
                                outputFile.setValue(row, col, (int)z1 / (int)z2);
                            }
                        }
                    }
                ...
                int const2 = (int)constant2;
                for (row = 0; row < rows; row++) {
                    data1 = inputFile1.getRowValues(row);
                    for (col = 0; col < cols; col++) {
                        z1 = data1[col];
                        if (z1 != noData && constant2 != 0) {
                            outputFile.setValue(row, col, (int)z1 / const2);
                        }
                    }
                    progress = (int) (100f * row / (rows - 1));
                    if (progress != oldProgress) {
                ...
                for (row = 0; row < rows; row++) {
                    ...
                    for (col = 0; col < cols; col++) {
                        z2 = data2[col];
                        if (z2 != noData && z2 != 0) {
                            outputFile.setValue(row, col, const1 / (int)z2);
                        }
                    }
                    progress = (int) (100f * row / (rows - 1));
                    if (progress != oldProgress) {
