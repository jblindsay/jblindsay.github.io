 ...
 */
public class ExtractStreams implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Extracts stream grid cells from a flow accumulation raster.";
    }
    ...
    public void run() {
        ...
        if ((accumHeader == null) || (outputHeader == null)) {
            showFeedback("One or more of the input parameters have not been set properly.");
            return;
        }

        try {
            WhiteboxRaster image = new WhiteboxRaster(accumHeader, "r");
            ...
            for (row = 0; row < rows; row++) {
                ...
                for (col = 0; col < cols; col++) {
                    if (data[col] > channelizationThreshold && data[col] != noData) {
                        output.setValue(row, col, 1);
                    } else {
                        output.setValue(row, col, backValue);
                    }
                }
                if (cancelOp) {
                    cancelOperation();
