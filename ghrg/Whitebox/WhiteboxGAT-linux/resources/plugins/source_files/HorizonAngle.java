...
/**
 * This tool calculates the horizon angle (Sx) i.e., the maximum slope along a specified azimuth (0-360 degrees) for each grid cell in an input digital elevation model (DEM).
 *
 * @author Dr. John Lindsay email: jlindsay@uoguelph.ca
 */
public class HorizonAngle implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Calculates horizon angle (maximum upwind slope) for each grid "
                + "cell in an input digital elevation model (DEM).";
    }
    ...
    public void run() {
        ...
        try {
                ...
                for (col = 0; col < cols; col++) {
                    ...
                    if (currentVal != noData) {
                        ...
                        if (currentMaxVal != aSmallValue) {
                            ...
                            if (saveDistance) {
                                if (z < 0) { maxValDist = maxValDist * -1; }
            ...
            DEM.close();
            output.close();
            if (saveDistance) { outputDist.close(); }
