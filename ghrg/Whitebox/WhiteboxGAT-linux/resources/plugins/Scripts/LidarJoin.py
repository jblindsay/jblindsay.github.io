...
name = "LidarJoin" 
descriptiveName = "Join (LiDAR)" 
description = "Joins LiDAR (LAS) files together" 
...
class PluginTool(ActionListener):
	def __init__(self, args):
		if len(args) != 0:
			...
			self.sd.addDialogMultiFile("Select the input LAS files", "Input LAS Files:", "LAS Files (*.las), LAS")
			self.sd.addDialogFile("Output LAS file", "Output LAS File:", "save", "LiDAR Files (*.las), LAS", True, False)
	...
	def execute(self, args):
		try:
			...
			cmd = "." + os.path.sep + "NativePlugins" + os.path.sep + "lidar_join{}".format(ext)
			cmd += ' -i=\"{}\"'.format(inputfiles)
