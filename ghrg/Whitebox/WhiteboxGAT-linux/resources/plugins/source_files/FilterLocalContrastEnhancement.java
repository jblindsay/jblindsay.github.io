/*
 * Copyright (C) 2011-2012 Elnaz Shokouhi and Dr. John Lindsay <jlindsay@uoguelph.ca>
 *
...
public class FilterLocalContrastEnhancement implements WhiteboxPlugin {
    ...
    public String getName() {
        return "FilterLocalContrastEnhancement";
    }
    ...
    public String getDescriptiveName() {
        return "Local Contrast Enhancement Filter";
    }
    ...
    public String getToolDescription() {
        return "Performs a local contrast enhancement filter on an image.";
    }
    ...
    public void run() {
        ...
        if (alpha < 0 || alpha > 1.0) {
            showFeedback("The alpha parameter should be within the range of \n0.0 and 1.0. Please reset and try again.");
            return;
        }
        globalMean = Double.parseDouble(args[5]);
        predefinedStdDev = Double.parseDouble(args[6]);
        filterRounded = Boolean.parseBoolean(args[7]);
        reflectAtBorders = Boolean.parseBoolean(args[8]);
                    ...
                    if (z != noData) {
                        ...
                        if (n > 2) {
                            ...
                            if (stdDev > 0) {
                                ...
                                if (alpha >= 0 && alpha <= 1) {
                                    yij = (alpha * globalMean) + ((1 - alpha) * average) + (z - average) * (predefinedStdDev / (stdDev + 1));
                                    outputFile.setValue(row, col, yij);
                                }
                            } else {
