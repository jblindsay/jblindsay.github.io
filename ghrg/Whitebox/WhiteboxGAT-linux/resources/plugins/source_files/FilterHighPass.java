...
/**
 * This tool performs a high-pass filter on a raster image, which can be used to emphasize the short-range variability in an image.
 * @author Dr. John Lindsay email: jlindsay@uoguelph.ca
 */
public class FilterHighPass implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Performs a high-pass filter on an image.";
    }
    /**
                ...
                for (col = 0; col < cols; col++) {
                    ...
                    if (z != noData) {
                        ...
                        for (a = 0; a < numPixelsInFilter; a++) {
                            ...
                            if (val != noData) {
                                n += 1 * filterShape[a];
                                sum += val * filterShape[a];
                            }
                        }
                        
                        if (n > 0) {
                            outputFile.setValue(row,col, z - (sum / n));
                        } else {
