...
def name = "Closing"
def descriptiveName = "Closing"
def description = "Performs a closing operation (mathematical morphology) on an image."
...
public class Closing implements ActionListener {
	...
	public Closing(WhiteboxPluginHost pluginHost, 
	...
	private void execute(String[] args) {
		try {
			...
			boolean reflectAtEdge = Boolean.parseBoolean(args[4])
			...
			for (int row = 0; row < rows; row++) {
				for (int col = 0; col < cols; col++) {
  					...
  					if (z != nodata) {
                        ...
                        if (n > 0) {
                            temp.setValue(row, col, maxValue)
                        }    
  					}
  				}
			...
			for (int row = 0; row < rows; row++) {
				for (int col = 0; col < cols; col++) {
  					...
  					if (z != nodata) {
                        ...
                        if (n > 0) {
                            output.setValue(row, col, minValue)
                        }    
  					}
  				}
  				progress = (int)(100f * row / (rows - 1))
...
if (args == null) {
	...
	def f = new Closing(pluginHost, args, name, descriptiveName)
}
