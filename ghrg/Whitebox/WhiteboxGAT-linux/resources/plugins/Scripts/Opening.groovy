...
def name = "Opening"
def descriptiveName = "Opening"
def description = "Performs an opening operation (mathematical morphology) on an image."
...
public class Opening implements ActionListener {
	...
	public Opening(WhiteboxPluginHost pluginHost, 
			...
			for (int row = 0; row < rows; row++) {
				for (int col = 0; col < cols; col++) {
  					...
  					if (z != nodata) {
                        ...
                        if (n > 0) {
                            output.setValue(row, col, maxValue)
                        }    
  					}
  				}
  				progress = (int)(100f * row / (rows - 1))
...
if (args == null) {
	...
	def f = new Opening(pluginHost, args, name, descriptiveName)
}
