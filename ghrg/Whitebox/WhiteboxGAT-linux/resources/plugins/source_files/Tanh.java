...
/**
 * This tool creates a new raster in which each grid cell is equal to the hyperbolic tangent of the corresponding grid cell in an input raster.
 *
 * @author Dr. John Lindsay email: jlindsay@uoguelph.ca
 */
public class Tanh implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Returns the hyperbolic tangent (tanh) of each values in a raster.";
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (inputFile.getZUnits().toLowerCase().contains("rad")) { // radians
                multiplier = 1;
            }
            ...
            for (row = 0; row < rows; row++) {
                ...
                for (col = 0; col < cols; col++) {
                    z = data[col];
                    if (z != noData) {
                        outputFile.setValue(row, col, Math.tanh(z * multiplier));
                    } else {
