...
def fileTypeName = "MapWindow ASCII Grid"
def isRasterFormat = true
def interopPluginType = InteropPluginType.importPlugin;

public class ImportMapWindowAsciiGrid implements ActionListener {
    private WhiteboxPluginHost pluginHost
    private ScriptDialog sd;
    private String descriptiveName
	
    public ImportMapWindowAsciiGrid(WhiteboxPluginHost pluginHost, 
        ...
        if (args.length > 0) {
            ...
            sd.addDialogMultiFile("Select the input MapWindow ASCII grid files", "Input MapWindow ASCII Raster Grid Files:", "MapWindow ASCII Grid (*.txt; *.asc), TXT, ASC")
    ...
    private void execute(String[] args) {
        try {
            ...
            for (i = 0; i < numFiles; i++) {
				...
				double cellsizeX = -32768.0;
				double cellsizeY = -32768.0;
				String delimiter = " "
                int row = 0
                int col = 0
                oldProgress = -1
				...
				arcFile.eachLine { line -> 
					...
					if (str.startsWith("ncols")) {
						...
						cellsizeX = Double.parseDouble(str.replace("dx", "").trim())
						if (cellsizeY > -32768) {
							if (xllcorner != null) {
	                            east = xllcorner + cols * cellsizeX;
	                            west = xllcorner;
	                            south = yllcorner;
	                            north = yllcorner + rows * cellsizeY;
	                        } else {
	                            east = xllcenter - (0.5 * cellsizeX) + cols * cellsizeX;
	                            west = xllcenter - (0.5 * cellsizeX);
	                            south = yllcenter - (0.5 * cellsizeY);
	                            north = yllcenter - (0.5 * cellsizeY) + rows * cellsizeY;
	                        }
						}
					} else if (str.startsWith("dy")) {
						cellsizeY = Double.parseDouble(str.replace("dy", "").trim())
						if (cellsizeX > -32768) {
							if (xllcorner != null) {
	                            east = xllcorner + cols * cellsizeX;
	                            west = xllcorner;
...
if (args == null) {
    ...
    def f = new ImportMapWindowAsciiGrid(pluginHost, args, name, descriptiveName)
}
