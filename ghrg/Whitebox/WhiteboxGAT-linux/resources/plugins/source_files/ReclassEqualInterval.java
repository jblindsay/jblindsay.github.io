 ...
 */
public class ReclassEqualInterval implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
        return "Reclassifies the values in a raster image based on equal-ranges.";
    }
    ...
    public void run() {
        ...
        WhiteboxRasterInfo image;
        WhiteboxRaster output;
        int cols, rows, col, row, a, progress;
        double noData;
        String inputHeader;
        String outputHeader;
        double[] data;
        double reclassInterval = 0;
        double startingValue = 0;
        double endingValue = Float.POSITIVE_INFINITY;
        double z;
        ...
        inputHeader = args[0];
        outputHeader = args[1];
        reclassInterval = Double.parseDouble(args[2]);
        startingValue = Double.parseDouble(args[3]);
        if (!args[4].isEmpty() && !args[4].toLowerCase().contains("not specified")) {
            endingValue = Double.parseDouble(args[4]);
        }
        ...
        try {
            ...
            image = new WhiteboxRasterInfo(inputHeader);
            ...
            for (row = 0; row < rows; row++) {
                ...
                for (col = 0; col < cols; col++) {
                    if (data[col] != noData) {
                        if (isBetween(data[col], startingValue, endingValue)) {
                            z = Math.floor(data[col] / reclassInterval) * reclassInterval;
